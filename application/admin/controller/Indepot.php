...
class Indepot extends Base
{
    ...
    public function index()
    {
		...
		//总记录数
		$count = model('Indepot')->where($where)->count();
    ...
    public function save()
    {
        ...
        // 验证数据
        $validate = validate('Indepot');
        $res = $validate->check( $data );
		$res || returnJson($validate->getError(),-100);

        // 添加入库更新库存
        return (new \app\admin\util\Depot)->in_add($data);
    }
    ...
    public function delete($id)
    {
        ...
        foreach ($id as $k => $v) {
            ...
            $row = model('IndepotMain')->where(['fid' => $v])->select();

            model('Stock')->resetNum($v,$row);
            model('Indepot')->where(['id' => $v])->delete();
            model('IndepotMain')->where(['fid' => $v])->delete();
        }
        return ['error'	=>	0,'msg'	=>	'删除成功'];	

		// 判断是否删除成功
    	// if( model('Indepot')->destroy($id) ){
    	// 	return ['error'	=>	0,'msg'	=>	'删除成功'];
    	// }else{
